...
def player_attack(computer_grid):
    while True:
        try:
            ...
            print('''Invalid Input. Please enter a valid attack strategy.
            Your input must include a letter,
            followed by a number (e.g., A5).''')
...
def start_game():
    ...
    grid_size = 6
    player_grid = [[SYMBOL_EMPTY for _ in range(grid_size)]
                   for _ in range(grid_size)]
    computer_grid = [[SYMBOL_EMPTY for _ in range(grid_size)]
                     for _ in range(grid_size)]
    ...
    while player_no_of_hits < 5 and computer_no_of_hits < 5:
        ...
        print_grid([[SYMBOL_EMPTY if cell == SYMBOL_COMPUTER
                    else cell for cell in row] for row in computer_grid])
        ...
        if player_no_of_hits == 4:
            print('''Well Done!
            You've sunk your opponent's battleships.
            You Win!''')
            break
